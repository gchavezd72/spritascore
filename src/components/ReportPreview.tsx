...
export function ReportPreview({ result }: ReportPreviewProps) {
  ...
  const calc = getCalculatorById(localizedResult.calculatorId);
  const probable = localizedResult.cost.probable ?? localizedResult.cost.annual ?? 0;
  const rp = t.reportPreview;
  const rc = t.recommendationCard;

  const handlePrint = () => {
    trackEvent("report_downloaded", {
      calculator: localizedResult.calculatorId,
      resultId: localizedResult.id,
      locale: reportLocale,
    });
    window.print();
  };
  ...
  return (
    <div id="report-content" className="bg-white text-slate-900 p-8 max-w-4xl mx-auto print:p-0" lang={reportLocale}>
      <div className="flex justify-between items-start mb-8 border-b pb-6 print:mb-4">
        ...
        <div className="text-right text-sm text-slate-500">
          <p>
            {new Date(localizedResult.createdAt).toLocaleDateString(localeDateTag(reportLocale), {
              dateStyle: "long",
            })}
          </p>
          <p>ID: {localizedResult.id.slice(0, 12)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 p-4 bg-slate-50 rounded-lg">
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-slate-900">
            {getRiskLevelLabel(localizedResult.riskLevel, reportLocale)}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-500 uppercase">{rp.probableCost}</p>
          <p className="text-2xl font-bold text-brand-green">
            {formatCurrency(probable, localizedResult.currency)}
          </p>
        </div>
      </div>

      <h3 className="text-lg font-bold text-brand-navy mb-3">{rp.calculatorUsed}</h3>
      <p className="text-slate-700 mb-6">{calc ? tr(calc.title, reportLocale) : ""}</p>
      ...
      <ul className="list-disc pl-5 mb-6 space-y-1 text-slate-700">
        {localizedResult.riskFactors.map((f, i) => (
          <li key={i}>{f}</li>
        ))}
      </ul>
      ...
      <table className="w-full mb-6 text-sm border-collapse">
        <tbody>
          {Object.entries(localizedResult.impactMatrix).map(([key, value]) => (
      ...
      <table className="w-full mb-6 text-sm border-collapse">
        <tbody>
          {localizedResult.cost.items.map((item, i) => (
            <tr key={i} className="border-b border-slate-200">
              <td className="py-2 text-slate-600">{item.label}</td>
              <td className="py-2 text-right font-semibold">
                {formatCurrency(item.value, localizedResult.currency)}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className="space-y-3 mb-6">
        {localizedResult.recommendations.map((rec, i) => (
          <div key={rec.id} className="border-l-4 border-brand-green pl-4 py-1">
            <p className="font-semibold text-brand-navy">
              {i + 1}. {tr(rec.title, reportLocale)}
            </p>
            <p className="text-sm text-slate-600">{tr(rec.description, reportLocale)}</p>
            <p className="text-xs text-slate-400 mt-1">
              {rc.priority}: {rc.priorityLabels[rec.priority]} | {rc.timeframe}:{" "}
              {rc.timeframeLabels[rec.timeframe]} | {rc.area}: {rc.areaLabels[rec.area]}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-8">
        {localizedResult.immediateActions.length > 0 && (
          <div>
            <p className="font-semibold text-sm text-risk-critical mb-1">{rp.immediate}</p>
            <ul className="text-sm text-slate-700 list-disc pl-5">
              {localizedResult.immediateActions.map((a) => (
                <li key={a.id}>{tr(a.title, reportLocale)}</li>
              ))}
            </ul>
          </div>
        )}
        {localizedResult.actions30Days.length > 0 && (
          <div>
            <p className="font-semibold text-sm text-brand-green mb-1">{rp.days30}</p>
            <ul className="text-sm text-slate-700 list-disc pl-5">
              {localizedResult.actions30Days.map((a) => (
                <li key={a.id}>{tr(a.title, reportLocale)}</li>
              ))}
            </ul>
          </div>
        )}
        {localizedResult.actions90Days.length > 0 && (
          <div>
            <p className="font-semibold text-sm text-brand-navy mb-1">{rp.days60to90}</p>
            <ul className="text-sm text-slate-700 list-disc pl-5">
              {localizedResult.actions90Days.map((a) => (
                <li key={a.id}>{tr(a.title, reportLocale)}</li>
              ))}
            </ul>
          </div>
